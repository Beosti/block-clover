package com.yuanno.blockclover.entity;
...
import com.yuanno.blockclover.events.projectiles.ProjectileBlockEvent;
import com.yuanno.blockclover.events.projectiles.ProjectileHitEvent;
import com.yuanno.blockclover.events.projectiles.ProjectileShootEvent;
import com.yuanno.blockclover.init.ModEntityPredicates;
...
public class SpellProjectileEntity extends ThrowableEntity
{
	...
	private static final DataParameter<Boolean> IS_PHYSICAL = EntityDataManager.defineId(SpellProjectileEntity.class, DataSerializers.BOOLEAN);
	...
	public SpellProjectileEntity(EntityType type, World world, double x, double y, double z)
	{
		super(type, x, y, z, world);
	}

	public SpellProjectileEntity(EntityType type, World world, LivingEntity thrower)
	{
	...
	protected void onHit(RayTraceResult hit)
	{
		...
		if (hit.getType() == RayTraceResult.Type.ENTITY)
		{
			...
			if (entityHit.getEntity() instanceof LivingEntity && this.getOwner() != null && this.getOwner() instanceof LivingEntity)
			{
				...
				else if (entityHit.getEntity() instanceof SpellProjectileEntity)
				{
					SpellProjectileEntity entity = (SpellProjectileEntity) entityHit.getEntity();
	...
	public void onProjectileCollision(SpellProjectileEntity owner, SpellProjectileEntity target)
	{
	...
	{
		return this.getEntityData().get(OWNER) != null
				&& this.level.getEntity(this.getEntityData().get(OWNER)) instanceof LivingEntity ? (LivingEntity) this.level.getEntity(this.getEntityData().get(OWNER)) : super.getOwner();
	}
