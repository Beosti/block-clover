...
public class RenderHelper {
    ...
    public static void drawTooltip(String attribute, MatrixStack mStack, java.util.List<? extends ITextProperties> textLines, int mouseX, int mouseY, int screenWidth, int screenHeight, int maxTextWidth, int backgroundColorStart, int backgroundColorEnd, int borderColorStart, int borderColorEnd, FontRenderer font)
    {
        ItemStack stack = ItemStack.EMPTY;
        if (!textLines.isEmpty())
        {
            ...
            int nameWidth = font.width(attribute);
            Color iconColor = Beapi.hexToRGB("#333333");
            Set<ResourceLocation> coloredIcons = new HashSet<>();
            Set<ResourceLocation> staticIcons = new HashSet<>();
